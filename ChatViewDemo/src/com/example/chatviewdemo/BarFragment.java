...
public class BarFragment extends Fragment {
	...
	private final String[] mLabelsOne = { "10-15", "15-20", "20-25", "25-30",
			"30-35" };
	...
	private final String[] mLabelsThree = { "", "", "", "", "", "", "", "", "",
			"", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",
			"", "", "", "", "", "", "", "", "", "", "", "", "", "" };
	private final float[] mValuesThree = { 2.5f, 3.7f, 4f, 8f, 4.5f, 4f, 5f,
			7f, 10f, 14f, 12f, 6f, 7f, 8f, 9f, 8f, 9f, 8f, 7f, 6f, 5f, 4f, 3f,
			4f, 5f, 6f, 7f, 8f, 9f, 11f, 12f, 14, 13f, 10f, 9f, 8f, 7f, 5f, 4f,
			6f };
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mPlayOne.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				/*
				 * if(mUpdateOne) updateChart(0, mChartOne, mPlayOne); else {
				 * dismissChart(0, mChartOne, mPlayOne); } mUpdateOne =
				 * !mUpdateOne;
				 */
		...
		// Init second chart
		mUpdateTwo = true;
		mChartTwo = (HorizontalBarChartView) layout
				.findViewById(R.id.barchart2);
	 ...
	 *            Play button
	 */
	private void updateChart(final int tag, final ChartView chart,
			ImageButton btn) {
	...
	public void produceOne(ChartView chart, Runnable action) {
		...
		Tooltip tooltip = new Tooltip(getActivity(),
				R.layout.barchart_one_tooltip);
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			tooltip.setEnterAnimation(PropertyValuesHolder.ofFloat(View.ALPHA,
					1));
			tooltip.setExitAnimation(PropertyValuesHolder
		...
		barChart.setBorderSpacing(5).setAxisBorderValues(0, 10, 2)
				.setGrid(BarChartView.GridType.FULL, 10, 10, gridPaint)
				.setYAxis(false).setXLabels(XController.LabelPosition.OUTSIDE)
		...
		barChart.show(new Animation().setOverlap(.5f, order).setEndAction(
				chartOneAction))
		// .show()
		;
	}
	...
	public void dismissOne(ChartView chart, Runnable action) {
		...
		dismissTooltipOne();
		int[] order = { 0, 4, 1, 3, 2 };
		chart.dismiss(new Animation().setOverlap(.5f, order).setEndAction(
				action));
	}
	...
	private void showTooltipOne() {
		...
		for (int i = 0; i < areas.size(); i++) {
			for (int j = 0; j < areas.get(i).size(); j++) {
				...
				Tooltip tooltip = new Tooltip(getActivity(),
						R.layout.barchart_one_tooltip, R.id.value);
				tooltip.prepare(areas.get(i).get(j), mValuesOne[i][j]);

				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
					tooltip.setEnterAnimation(PropertyValuesHolder.ofFloat(
							View.ALPHA, 1));
					tooltip.setExitAnimation(PropertyValuesHolder.ofFloat(
							View.ALPHA, 0));
				}
	...
	public void produceTwo(ChartView chart, Runnable action) {
		...
		horChart.setOnEntryClickListener(new OnEntryClickListener() {
			@Override
			public void onClick(int setIndex, int entryIndex, Rect rect) {
				mTextViewTwo.setText(Integer
						.toString((int) mValuesTwo[entryIndex]));
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB_MR1) {
		...
		horChart.setBorderSpacing(0).setAxisBorderValues(0, 100, 5)
				.setXAxis(false).setYAxis(false)
				.setLabelsColor(Color.parseColor("#FF8E8A84"))
				.setXLabels(XController.LabelPosition.NONE);

		int[] order = { 4, 3, 2, 1, 0 };
		horChart.show(new Animation().setOverlap(.5f, order).setEndAction(
				action));
	}
	...
	public void produceThree(ChartView chart, Runnable action) {
		...
		Tooltip tip = new Tooltip(getActivity(),
				R.layout.barchart_three_tooltip);
		...
		barChart.setXLabels(AxisController.LabelPosition.NONE)
				.setYLabels(AxisController.LabelPosition.NONE).setXAxis(false)
				.setYAxis(false);

		Animation anim = new Animation().setEasing(new ElasticEase())
				.setEndAction(action);
	...
	public void updateThree(ChartView chart) {
		...
		float[] values = { 3.5f, 4.7f, 5f, 9f, 5.5f, 5f, 6f, 8f, 11f, 13f, 11f,
				7f, 6f, 7f, 10f, 11f, 12f, 9f, 8f, 7f, 6f, 5f, 4f, 3f, 6f, 7f,
				8f, 9f, 10f, 12f, 13f, 11, 13f, 10f, 8f, 7f, 5f, 4f, 3f, 7f };
	...
	public void dismissThree(ChartView chart, Runnable action) {
		...
		chart.dismissAllTooltips();
		chart.dismiss(new Animation().setEasing(new ElasticEase())
				.setEndAction(action));
	}
